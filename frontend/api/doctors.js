...
export default async function handler(req, res) {
  ...
  try {
    ...
    const action = req.query.action || 
                   (pathname === '/api/doctor/dashboard' ? 'doctor-dashboard' : null) ||
                   (pathname.startsWith('/api/doctor/clinics/') ? 'update-clinic' : null) ||
                   (pathname.startsWith('/api/doctors/') && pathname !== '/api/doctors' ? 'details' : null);

    // 1. DOCTOR DASHBOARD
    if (action === 'doctor-dashboard') {
      if (req.method !== 'GET') {
    ...
    if (action === 'update-clinic') {
      ...
      const clinicId = req.query.clinicId || pathname.split('/').pop();
    ...
    if (action === 'details') {
      ...
      const doctorId = req.query.id || pathname.split('/').pop();
    ...
    // 4. LIST DOCTORS (GET /api/doctors) - Default action if action is not matched or pathname is base
    if (!action || pathname === '/api/doctors') {
      if (req.method !== 'GET') {
        res.setHeader('Allow', ['GET']);
        return res.status(405).json({ error: `Method ${req.method} Not Allowed` });
      }
    ...
    console.error('API Error in doctors handler:', error);
