...
export default async function handler(req, res) {
  ...
  if (pathname.startsWith('/api/admin/users/') && pathname.endsWith('/role')) {
    if (req.method !== 'PUT') {
    ...
    const targetUserId = parts[4]; // parts: ['', 'api', 'admin', 'users', ':id', 'role']
    const { role } = req.body;
  ...
  if (pathname === '/api/admin/assistants/assign') {
    if (req.method !== 'POST') {
